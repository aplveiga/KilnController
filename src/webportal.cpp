...
void WirelessManager::handleRoot() {
    ...
    server.sendContent(F(
        ...
        "const sorted=d.entries.slice().sort((a,b)=>b.sequence-a.sequence);"
        "const end=Math.min(10,sorted.length);"
        "for(let i=0;i<end;i++){const e=sorted[i];const row=t.insertRow();const dt=new Date(e.timestamp*1000).toLocaleString();row.insertCell(0).textContent=dt;row.insertCell(1).textContent=e.temperature.toFixed(1);row.insertCell(2).textContent=e.setpoint.toFixed(1);row.insertCell(3).textContent=e.target.toFixed(1);row.insertCell(4).textContent=e.program;row.insertCell(5).textContent=e.status;}"
        "});"
        "}"
