...
void WirelessManager::handleRoot() {
    ...
    server.sendContent(F(
        ...
        "</div>"
        "<h3>SSR Diagnostics</h3>"
        "<div class='dashboard-grid'>"
        "<div class='dashboard-item'><div class='dashboard-item-label'>SSR State</div><div class='dashboard-item-value' id='dash-ssr' style='font-size:18px'>OFF</div></div>"
        "<div class='dashboard-item'><div class='dashboard-item-label'>PID Output</div><div class='dashboard-item-value' id='dash-pid'>0%</div></div>"
        "<div class='dashboard-item'><div class='dashboard-item-label'>Window Position</div><div class='dashboard-item-value' id='dash-window' style='font-size:16px'>0 ms</div></div>"
        "<div class='dashboard-item'><div class='dashboard-item-label'>Time Since Change</div><div class='dashboard-item-value' id='dash-change' style='font-size:16px'>0 ms</div></div>"
        "</div>"
        "<h3>Controls</h3>"
    ...
    server.sendContent(F(
        ...
        "document.getElementById('dash-ssr').textContent=d.ssrStatus?'ON':'OFF';"
        "document.getElementById('dash-ssr').style.color=d.ssrStatus?'#4caf50':'#f44336';"
        "document.getElementById('dash-pid').textContent=(d.pidOutput||0).toFixed(1)+'%';"
        "document.getElementById('dash-window').textContent=(d.windowPos||0)+' / '+(d.windowSize||0)+' ms';"
        "document.getElementById('dash-change').textContent=(d.timeSinceSSRChange||0)+' / '+(d.ssrRateLimit||0)+' ms';"
        "});"
        "}"
